...
function initializeAdminHotspot() {
    ...
    // Posicionar hotspot exatamente sobre o logo, sem cobrir o restante da navbar
    function positionHotspot() {
        if (!brandLogo) return;
        const rect = brandLogo.getBoundingClientRect();
        const containerRect = document.getElementById('brand-wrapper').getBoundingClientRect();
    ...
    window.addEventListener('scroll', positionHotspot, { passive: true });
    ...
    if (brandLogo) {
        let pressTimer;
        brandLogo.addEventListener('mousedown', function(e) {
            pressTimer = setTimeout(() => openAdminPrompt(), 600);
        });
        ['mouseup','mouseleave'].forEach(evt => brandLogo.addEventListener(evt, function() {
            clearTimeout(pressTimer);
        }));
        // Suporte mobile (touch)
        brandLogo.addEventListener('touchstart', function() {
            pressTimer = setTimeout(() => openAdminPrompt(), 600);
        });
        ['touchend','touchcancel'].forEach(evt => brandLogo.addEventListener(evt, function() {
            clearTimeout(pressTimer);
        }));
    }
}
